...
function isCreditCardNumber(ccn) {
  const arr = String(ccn).split('').map((item) => +item);
...
function getDigitalRoot(num) {
  /* eslint no-eval: 0 */
  const res = eval(num.toString().split('').join('+'));
...
function isBracketsBalanced(str) {
  ...
  while (true) {
    ...
    if (res === newStr) return res.length === 0;
    newStr = res;
  }
}
...
function getCommonDirectoryPath(pathes) {
  let answer = '';
  let path = pathes;
  if (!path.every((el) => el[0] === '/')) return answer;
  path = path.map((e) => e.split('/'));
  if (path.every((el) => el[1] === path[0][1])) answer += `/${path[0][1]}/`; else return '/';
  if (path.every((el) => el[2] === path[0][2])) answer += `${path[0][2]}/`; else return answer;
  if (path.every((el) => el[3] === path[0][3])) answer += `${path[0][3]}/`; else return answer;
